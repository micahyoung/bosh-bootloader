	...
	Describe("lbs", func() {
		Context("when bbl'd up with terraform", func() {
			...
			FIt("prints out the currently attached lb names and urls", func() {
				...
				Expect(stdout).To(ContainSubstring("CF Router LB Name: some-router-lb-name\n"))
				Expect(stdout).To(ContainSubstring("CF Router LB URL: some-router-lb-url\n"))
				Expect(stdout).To(ContainSubstring("CF SSH Proxy LB Name: some-ssh-proxy-lb-name\n"))
				Expect(stdout).To(ContainSubstring("CF SSH Proxy LB URL: some-ssh-proxy-lb-url\n"))
				Expect(stdout).To(ContainSubstring("System Domain DNS servers: name-server-1. name-server-2.\n"))
			})
